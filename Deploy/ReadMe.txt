...
Update by Ar1z for Project Rubi-Ka patch (executable & database changes)
https://users.math.uoc.gr/~ar1z/index.html

Fixes by pzychotic for 18.8  Anarchy Online patch (executable changes)
...
for PRK it is the subfolder "\client" inside the folder where
you have unpacked and "installed" the PRK Client. In this subfolder
you find the Anarchy.exe and AnarchyOnline.exe.
...
History
-------
2.7.0       - Removed the depentacy to Berkley DB, Now it connects directly to PRK Database
			  You can remove libdb40.dll and AODatabase.dbd.
2.6.2		- Fixed the bug with Return Item missions (Project RK had changed it's id)
			- Added option to select the delay between missions in milliseconds. The default is 5000,
			  in order to be compatible with Project RK.
			- Added text in in the select PRK folder to make it easier to understand wich folder it is.
2.6.1		- Added 5" between missions to be compatible with Project RK
			- Removed Double selection for AO folder
2.6.0		- Modified the code to read from Project RK DB
			- Added sqlite 3 support
			- Removed C-Tree support
